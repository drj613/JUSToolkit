...
[here](../research/Koma-research.md)
...
> **NOTE**: The sprite IDs from `KShape` do not correspond to the order defined
> in the DSTX. `KShape` points to blocks of 48x48 in the order defined in the
> image DSIG, which may be different as the sprites defined here.
...
There are 8 groups. Each group is the number of tiles of the shape of the
sprite. The number of shapes means the posible combinations of the sprite. If
it's size 2 (group 1), we will have an horizontal shape and a vertical shape, so
group 1 will have 2 shapes.
