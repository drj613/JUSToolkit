...
- **`+0x1a8`** — ~~MoveInfo~~ **battle-character** pointer. Allocated via `0x02156A38`/`0x02156A58` (size `0x1F0` = 496 bytes); installed via setter `0x021570EC`. **(iteration 73: the call-site allocation tag reads `Battle_CharaCreate` / `BattleChara.cpp`. Every offset below `0x1F0` in this list is therefore ambiguous between this object and the outer one — `+0x84`, `+0x1B4`, `+0x1B8` are explicitly unresolved; `+0x1E0` is CONFIRMED to be the inner object's.)**
