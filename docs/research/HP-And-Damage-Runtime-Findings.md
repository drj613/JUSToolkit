...
This is the headline correction. Earlier docs said "HP is stored at 1/4 scale
(160 displayed = 40 stored)". That byte is real, but it is only the **high byte
of a 16-bit little-endian HP value**.
...
`9216 / 64 = 144`, which matches the independently-derived 4-koma Naruto HP of
144 from koma static analysis. Three independent values all land on exact
integers at scale 64, so the scale is not a coincidence.

**Why it matters:** damage of 6 or 1.25 displayed HP is *unrepresentable* at 1/4
scale, so any damage-formula work that reads only the high byte silently
truncates. All the sub-integer damage below was invisible before this.
...
is consistent with a 2/3 multiplier on blunt against Luffy, but this is **one
data point against one target and is not yet confirmed** — see open questions.
...
can take damage while benched. 1.250 = 80 raw units, which is not a multiple of
64 — further proof the value is genuine sub-integer fixed point and not a
rounding artifact.

Attempts to force a character *swap* with DOWN+B did not change which character
was active (opponent active HP stayed at Luffy's 152 baseline across 6 attempts).

## 3. Training mode heals to full — read damage per frame, not before/after

In Training, HP returns to full within a few frames of any hit: it steps back up
roughly one 1/4-unit (64 raw) every ~2 frames until full.

Consequences for any measurement harness:

- **Do not** diff HP before and after an exchange. It will read zero.
- Log HP **every frame** and take damage as `baseline − min(dip)`, or detect
...
Evidence: with the same savestate and adjacent characters, a probe pressing each
of A, B, X, Y, R, L twelve times produced **zero** opponent damage across all six
...
So: **a scripted attack plan must establish facing first.** Overlapping sprites
are not enough; the characters were visually on top of each other in the whiffing
runs.
...
3. **Why does DOWN+B hit a benched character for 1.250?** Splash? A support
   mechanic? A fixed fraction of the main hit (1.25 / 3.0 = 0.4167, not obviously
   clean)?
4. **What sets the 1/64 fraction?** With HP in 1/64 units, damage may be computed
   as a fraction and truncated. `fn_health_calc` at `0x020784FC` is the place to
   look.
