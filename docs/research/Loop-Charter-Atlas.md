...
file (it may have been edited between wakes) and `scripts/analysis/loop-state-atlas.json`,
do ONE task, commit, update state, then schedule the next wake.

## Mission

Two goals, in this order of finish-ability:
...
1. **Koma sprint (finish first, it's bounded):** understand the deckbuilding / koma system
   ...
   lives), and what is still unknown. Existing partial docs: `Deck-System.md`,
...
2. **Combat engine (open-ended, runs until stopped):** deepen `Battle-Engine-Map.md`.
   Priorities: (a) the two least-resolved subsystems, hitbox-priority and physics-writers;
   (b) Tier-2 plan tasks D0.1 and D0.2 from `docs/design/Decomp-Tier2-Plan.md` (overlay map,
   function denominator) — they unblock everything later; (c) any GDB-Validation-Queue card
...
- **Protect the main context.** The orchestrator never Reads disassembly, large JSON, or
  binary dumps directly. All heavy reading happens in subagents that return short summaries.
...
- **Synthesis / adversarial verification**: `general-purpose` with `model: "opus"`. Reuse the
  ...
  lenses (disasm re-check, aliasing/alternative-explanation, data-consistency) and score
  CONFIRMED only on agreement.
- **Second opinion**: when a subsystem conclusion is about to be written into canon docs, or
  when stuck two wakes in a row on the same question, invoke the `codex:rescue` skill (or the
...
- **Editing pass (mandatory, opus-4-6 specifically):** any time a doc under `docs/` is
  created or substantially edited, run it through a headless claude call pinned to
  ...
  Draft to the scratchpad, diff EDITED against DRAFT to confirm no addresses/hex values were
  dropped (e.g. compare `grep -oE '0x[0-9A-Fa-f]+' | sort` output of both), then write the
  edited version into `docs/`. If the call fails (model unavailable, auth), keep the draft,
  note the skipped pass in the commit message, and retry next wake.
...
Seed queue (do in order unless blocked; append new tasks as they're discovered):
