...
- **Convergent verification** (owner rule, standing as of 2026-08-18). For every load-bearing
  decode or address claim, find a second method that uses a **different representation** —
  relative displacement vs. absolute address, encoding-table arithmetic vs. empirical ROM
  sweep, byte offset vs. instruction index. Two representations agreeing can't share a bias,
  so cross-representation match is the strongest confirmation static-only rules allow.
  Disagreement is just as valuable: one side is wrong, and the mismatch usually reveals which.
  Never substitute a second run of the same tool or a checker working from the same angle.
  - Worked example, iteration 149: Codex decoded `EB00034E` relatively as a branch to
    ...
    `0x0215305C`; `0x0215231C + 0xD40` = `0x0215305C`. Neither tool alone could confirm the
    window's placement — their agreement did.
  - **Ask the independent checker BEFORE forming your own conclusion**, not after. Same
    iteration: concluding first would have buried a mis-transcribed halfword (`0x6668` for
    `0x66bc`). Asking first surfaced the inconsistency instead of decoding around it.
