...
A single move can have **multiple collision entries** in the character's collision file.
Each entry defines a separate hitbox with its own:
...
This allows the game to create complex attack patterns without special-case code.
...
Two separate hitbox entries with different positions. The close hitbox has higher
damage. Both are active simultaneously during the beam attack.
...
|------|---------|--------|-----------|
| fwd Y | Zone damage | 3 close, 2 far | Two hitboxes at different offsets |
| Y | Repeating + finisher | 2×N + 9 | Tick hitbox + final hitbox |
| air B | Multi-part | 15 ball, 5 body | Two simultaneous hitboxes |
| up Y | Multi-hit combo | 3×6 + 10 | Sequential frameStart entries |
...
1. **Damage formula still applies:** Each hitbox independently uses `damage = d1/5 + (tier-2)`
2. **damageFlags is key:** Different damageFlags values point to different jpower entries
3. **Animation controls timing:** frameStart and durationMult determine when/how often hits register
4. **No special code needed:** Complex attacks emerge from simple collision entry combinations
...
**Step 3:** Analyze JSON output
The JSON will show all collision entries with their exact values for:
...
- [jpower-Block-Pattern-Analysis.md](./jpower-Block-Pattern-Analysis.md) - jpower entry analysis
