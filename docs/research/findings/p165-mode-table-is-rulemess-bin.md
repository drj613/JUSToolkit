...
**`jus-vkj` was retracted (runtime, same day).** `advance(N)` is accurate — requested 2300 gives an observed 2301. The real mechanism is that the emulator **free-runs at ~60 fps whenever no plan is executing**, so a framecount delta spanning the caller's own work overcounts. Nothing here was affected either way: every number used is a single RAM read or a framecount the emulator reported at capture. The `4463` figure stands, and it stands for a stated reason — the game cannot tell a plan frame from a free-run frame, so the TIME display counts down on both.
